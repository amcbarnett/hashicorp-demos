...
VAULT_POD=$(oc get pods --namespace vault-deploy -l "app=vault" -o jsonpath="{.items[0].metadata.name}")
VAULT_SERVICE=$(oc get svc -l app=vault -o jsonpath="{.items[0].metadata.name}")

## Using Command Line
#oc exec -it vault-0 -c vault -- /bin/sh

#oc exec -it ${VAULT_POD} -c ${VAULT_SERVICE} -- /bin/sh export VAULT_ADDR=http://127.0.0.1:8200

oc exec -it ${VAULT_POD} -c ${VAULT_SERVICE} -- /bin/sh -c 'VAULT_ADDR=http://localhost:8200 vault operator init -key-shares=1 -key-threshold=1'
...
for v in `oc get pods | grep vault | cut -f1 -d' '`; do oc exec -ti $v -c vault -- /bin/sh -c "VAULT_ADDR=http://localhost:8200 vault operator unseal ${VAULT_UNSEAL}
"; done

for v in `oc get pods | grep vault | cut -f1 -d' '`; do oc exec -ti $v -c vault -- /bin/sh -c 'VAULT_ADDR=http://localhost:8200 vault status'; done


#export VAULT_TOKEN=$(oc logs $VAULT_POD vault | grep 'Root Token' | cut -d' ' -f3)

#export VAULT_ADDR=http://127.0.0.1:8200

# run this on a second terminal
#oc port-forward $VAULT_POD 8200
...
#oc create clusterrolebinding add-on-cluster-admin --clusterrole=cluster-admin --serviceaccount=kube-system:default
#oc create clusterrolebinding role-tokenreview-binding --clusterrole=system:auth-delegator --serviceaccount=k8s-app1:vault-app

# 2. Grant service account ability to access the TokenReviewer API via RBAC


oc create serviceaccount k8s-app1
oc create serviceaccount k8s-app2
...
oc apply -f vault-serviceaccount.yml
...
# oc config view -o jsonpath='{range .clusters[*]}{.name}{"\t"}{.cluster.server}{"\n"}{end}'

/*
#export VAULT_SA_NAME="$(oc get serviceaccount vault-auth -o go-template='{{ (index .secrets 0).name }}')"
#export SA_JWT_TOKEN="$(oc get secret ${VAULT_SA_NAME} -o go-template='{{ .data.token }}' | base64 --decode)"
#export K8S_HOST="$(oc config view --raw \
#    -o go-template="{{ range .clusters }}{{ if eq .name \"${CLUSTER_NAME}\" }}{{ index .cluster \"server\" }}{{ end }}{{ end }}")"
#export K8S_CACERT="$(oc config view --raw \
#    -o go-template="{{ range .clusters }}{{ if eq .name \"${CLUSTER_NAME}\" }}{{ index .cluster \"certificate-authority-data\" }}{{ end }}{{ end }}" | base64 --decode)"

export VAULT_SA_NAME=$(oc get serviceaccount vault-auth -o jsonpath="{.secrets[*]['name']}")
export SA_JWT_TOKEN=$(oc get secret ${VAULT_SA_NAME} -o jsonpath="{.data.token}" | base64 --decode; echo)
export K8S_CACERT=$(oc get secret ${VAULT_SA_NAME} -o jsonpath="{.data['ca\.crt']}" | base64 --decode; echo)
#export K8S_HOST=$(oc exec -it vault-0 -c vault -- /bin/sh -c 'echo $KUBERNETES_SERVICE_HOST')
#echo $K8S_HOST | tr -d '\n'
#export K8S_HOST=$(oc exec -it vault-0 -c vault -- /bin/sh -c 'echo $KUBERNETES_SERVICE_HOST | tr -d \'\n\' ')
export K8S_HOST=$(oc exec -it vault-0 -c vault -- /bin/sh -c "echo \$KUBERNETES_SERVICE_HOST | tr -d '\n' ")

export VAULT_SERVICE=$(oc get svc -l app=vault -o jsonpath="{.items[0].metadata.name}")
...
oc run --generator=run-pod/v1 tmp  -i --tty --serviceaccount=vault-auth --image alpine

oc attach tmp -c tmp -i -t
...
oc run --generator=run-pod/v1 tmp-app1  -i --tty --serviceaccount=k8s-app1 --image alpine


oc attach tmp-app1 -c tmp-app1 -i -t
...
oc run --generator=run-pod/v1 tmp-app2  -i --tty --serviceaccount=k8s-app2 --image alpine

oc attach tmp-app2 -c tmp-app2 -i -t
